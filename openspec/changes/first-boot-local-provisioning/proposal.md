...
manually copied into `/data/config/` after boot. This makes fresh installs, reprovisioning, and future Overwatch
...
- **Affected storage layout**: `/data/config/` becomes the canonical home for imported `config.toml` and rendered
  Quadlet files
- **Affected operational workflows**: fresh flash, reprovisioning, bench setup, and future Overwatch alignment
