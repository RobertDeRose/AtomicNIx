...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.succeed("ATOMICNIX_BOOTSTRAP_POST_APPLY=/tmp/bootstrap-post-apply-fail first-boot-provision serve /tmp/bootstrap-root-api-fail /tmp/bootstrap-output-api-fail.toml --host 127.0.0.1 --port 18080 >/tmp/bootstrap.log 2>&1 & echo $! >/tmp/bootstrap.pid")
    gateway.wait_until_succeeds("ss -tln | grep ':18080'", timeout=30)
