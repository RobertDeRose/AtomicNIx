...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.succeed("ATOMICNIX_CHRONY_WAIT_TIMEOUT_SECONDS=1 ATOMICNIX_CHRONY_WAIT_ATTEMPTS=2 ATOMICNIX_ALLOW_UNSAFE_PATH=1 PATH=/tmp/quadlet-sync/bin:$PATH quadlet-sync >/tmp/quadlet-sync/output.log 2>&1 || (cat /tmp/quadlet-sync/output.log >&2; false)")
    ...
    gateway.succeed("ATOMICNIX_CHRONY_WAIT_TIMEOUT_SECONDS=1 ATOMICNIX_CHRONY_WAIT_ATTEMPTS=1 ATOMICNIX_ALLOW_UNSAFE_PATH=1 PATH=/tmp/quadlet-sync/bin:$PATH quadlet-sync >/tmp/quadlet-sync/rootful-only.log 2>&1 || (cat /tmp/quadlet-sync/rootful-only.log >&2; false)")
    gateway.succeed("grep '^sync-quadlet /data/config /etc/containers/systemd /var/lib/appsvc/.config/containers/systemd$' /tmp/quadlet-sync/provision.log")
    gateway.succeed("grep '^daemon-reload$' /tmp/quadlet-sync/systemctl.log")
    gateway.succeed("grep '^restart edgeproxy.service$' /tmp/quadlet-sync/systemctl.log")
