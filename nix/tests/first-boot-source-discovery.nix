...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.succeed("rm -rf /test-usb/usb-a/* /test-usb/usb-b/*")
    gateway.succeed("ATOMICNIX_CONFIG_ROOT=/tmp/bootstrap-root ATOMICNIX_FIRST_BOOT_SENTINEL=/tmp/bootstrap-sentinel ATOMICNIX_BOOT_SLOT=boot.a ATOMICNIX_USB_SEARCH_DIRS='/test-usb' PATH=/testbin:/run/current-system/sw/bin first-boot >/tmp/bootstrap-case.log 2>&1 & echo $! >/tmp/bootstrap-case.pid")
    ...
    gateway.succeed("rm -f /boot/config.toml /etc/atomicnix/fresh-flash /tmp/no-dev-fallback-sentinel")
    gateway.succeed("rm -rf /test-usb/usb-a/* /test-usb/usb-b/*")
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin ATOMICNIX_CONFIG_ROOT=/tmp/no-dev-fallback-root ATOMICNIX_FIRST_BOOT_SENTINEL=/tmp/no-dev-fallback-sentinel ATOMICNIX_BOOT_SLOT=boot.a ATOMICNIX_USB_SEARCH_DIRS='/test-usb' ATOMICNIX_BOOTSTRAP_HOST=127.0.0.1 ATOMICNIX_DEV_ENABLE_SSH_WAN=1 first-boot >/tmp/no-dev-fallback.log 2>&1 & echo $! >/tmp/no-dev-fallback.pid")
    gateway.succeed("sleep 3")
