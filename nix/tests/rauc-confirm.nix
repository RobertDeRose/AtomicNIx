...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.wait_for_unit("rauc.service")
    gateway.fail("PATH=${raucStub}/bin:${pkgs.jq}/bin:${pkgs.systemd}/bin:${pkgs.iproute2}/bin:${pkgs.coreutils}/bin:${pkgs.gnugrep}/bin:${forensicStub}/bin:$PATH ATOMICNIX_TEST_FAIL_MARK_GOOD=1 ATOMICNIX_VERIFICATION_SUSTAIN_DURATION=1 ATOMICNIX_VERIFICATION_CHECK_INTERVAL=1 ${verificationScript} >/tmp/os-verification-fail.log 2>&1")
