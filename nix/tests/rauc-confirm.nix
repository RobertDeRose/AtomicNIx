...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.wait_for_unit("chronyd.service")
    gateway.fail("(sleep 3; systemctl stop chronyd.service) & PATH=${raucStub}/bin:${pkgs.jq}/bin:${pkgs.systemd}/bin:${pkgs.iproute2}/bin:${pkgs.coreutils}/bin:${pkgs.gnugrep}/bin:$PATH ATOMICNIX_VERIFICATION_SUSTAIN_DURATION=5 ATOMICNIX_VERIFICATION_CHECK_INTERVAL=1 ${verificationScript} >/tmp/os-verification-chrony-regress.log 2>&1")
    ...
    gateway.fail("PATH=${raucStub}/bin:${pkgs.jq}/bin:${pkgs.systemd}/bin:${pkgs.iproute2}/bin:${pkgs.coreutils}/bin:${pkgs.gnugrep}/bin:$PATH ATOMICNIX_VERIFICATION_SUSTAIN_DURATION=1 ATOMICNIX_VERIFICATION_CHECK_INTERVAL=1 ${verificationScript} >/tmp/os-verification-health-required-type.log 2>&1")
    ...
    gateway.succeed("PATH=${raucStub}/bin:${pkgs.jq}/bin:${pkgs.systemd}/bin:${pkgs.iproute2}/bin:${pkgs.coreutils}/bin:${pkgs.gnugrep}/bin:$PATH ATOMICNIX_VERIFICATION_SUSTAIN_DURATION=1 ATOMICNIX_VERIFICATION_CHECK_INTERVAL=1 ${verificationScript} >/tmp/os-verification-invalid-gateway-type.log 2>&1")
